...
export function StudentHomeworkClient({ homeworkList: initialList, fixedSubjectId, fixedChapterId }: StudentHomeworkClientProps) {
  ...
  const todoHomework = homeworkList
    .filter(h => h.submissions.length === 0 && new Date(h.dueDate) >= now && !h.title.startsWith('[Reassigned]'))
                ...
                {userSubmission ? (
                  ...
                  <div className={`rounded-3xl border p-5 space-y-4 ${
                      ...
                      <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 space-y-3 mt-4">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            Your teacher has evaluated, marked corrections, and drawn feedback on your submitted answer sheets.
                          </p>
                        </div>
                    ...
                    <div className="bg-background/80 rounded-2xl p-4 border border-border/40 mt-4">
                      <span className="text-[10px] text-muted-foreground uppercase font-bold block mb-2">Your Original Submission</span>
